...
def test_character_filter(test_client):
    ...
    assert tf_resp.entities == [AttributeValueEntity(id="001", attributes={"foo": "bar"})]
...
def test_date_time(test_client):
    tf_req = EntityTransformRequest(
        ...
        attribute_transformers=[
            AttributeTransformerConfig(
                attribute_name="foo",
                transformers=[DateTimeTransformer(input_format="%d.%m.%Y", output_format="%Y-%m-%d")],
            )
        ],
    )
    ...
    assert tf_resp.config == tf_req.config
    assert tf_resp.entities == [AttributeValueEntity(id="001", attributes={"foo": "1998-06-29"})]
...
def test_mapping(test_client):
    ...
    assert tf_resp.config == tf_req.config
    assert tf_resp.entities == [AttributeValueEntity(id="001", attributes={"foo": "baz"})]
...
def test_mapping_with_default(test_client):
    tf_req = EntityTransformRequest(
        ...
        attribute_transformers=[
            AttributeTransformerConfig(
                attribute_name="foo",
                transformers=[MappingTransformer(mapping={"x": "y"}, default_value="baz")],
            )
        ],
    )
...
def test_normalize(test_client):
    tf_req = EntityTransformRequest(
        ...
        attribute_transformers=[
            AttributeTransformerConfig(attribute_name="foo", transformers=[NormalizationTransformer()])
        ],
    )
    ...
    assert tf_resp.config == tf_req.config
    assert tf_resp.entities == [AttributeValueEntity(id="001", attributes={"foo": "b ar"})]


def test_number(test_client):
    tf_req = EntityTransformRequest(
        config=TransformConfig(empty_value=EmptyValueHandling.ignore),
        entities=[AttributeValueEntity(id="001", attributes={"bar1": "0012.345", "bar2": "0012.345"})],
        attribute_transformers=[
            AttributeTransformerConfig(
                attribute_name="bar1",
    ...
    assert tf_resp.config == tf_req.config
    assert tf_resp.entities == [AttributeValueEntity(id="001", attributes={"bar1": "12", "bar2": "12.35"})]
...
def test_phonetic(test_client):
    tf_req = EntityTransformRequest(
        ...
        attribute_transformers=[
            AttributeTransformerConfig(
                attribute_name="foo",
                transformers=[PhoneticCodeTransformer(algorithm=PhoneticCodeAlgorithm.soundex)],
            )
        ],
    )
    ...
    assert tf_resp.config == tf_req.config
    assert tf_resp.entities == [AttributeValueEntity(id="001", attributes={"foo": "B600"})]


def test_global_transformers(test_client):
    tf_req = EntityTransformRequest(
        config=TransformConfig(empty_value=EmptyValueHandling.ignore),
        entities=[AttributeValueEntity(id="001", attributes={"bar1": "  12.345  ", "bar2": "  12.345  "})],
        attribute_transformers=[
            AttributeTransformerConfig(
                attribute_name="bar1",
                transformers=[NumberTransformer(decimal_places=2)],
        ...
        global_transformers=GlobalTransformerConfig(
            before=[NormalizationTransformer()], after=[CharacterFilterTransformer(characters=".")]
        ),
    )
    ...
    assert tf_resp.config == tf_req.config
    assert tf_resp.entities == [AttributeValueEntity(id="001", attributes={"bar1": "1235", "bar2": "12345"})]
