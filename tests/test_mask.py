...
def _generate_person():
    return AttributeValueEntity(
        ...
        attributes={
            ...
            "dateOfBirth": fake.unique.date_of_birth(
                tzinfo=datetime.timezone.utc, minimum_age=18, maximum_age=120
            ).strftime("%Y-%m-%d"),
...
def _construct_request_for(
    mask_config: MaskConfig, attributes: list[WeightedAttributeConfig] | list[StaticAttributeConfig] | None = None
) -> EntityMaskRequest:
...
@pytest.mark.parametrize(
    "filter_type",
    [CLKFilter(filter_size=2**10, hash_values=5), RBFFilter(hash_values=5, seed=727), CLKRBFFilter(hash_values=5)],
    ids=["clk", "rbf", "clkrbf"],
)
def test_mask_filter_types(test_client, filter_type):
    _ = _retrieve_bit_vectors_from_request(
        test_client,
        _construct_request_for(
            MaskConfig(
                token_size=2,
...
def test_different_vectors_with_different_token_size(test_client, filter_type, token_sizes):
    _assert_bit_vectors_unique_across_requests(
        test_client,
        [
            _construct_request_for(
                MaskConfig(
    ...
    _assert_bit_vectors_unique_across_requests(
        ...
        [
            _construct_request_for(
                MaskConfig(
                    token_size=2,
                    hash=HashConfig(function=HashFunction(algorithms=[HashAlgorithm.sha1]), strategy=hash_strategy),
                    filter=filter_type,
                    padding="_",
                )
            )
...
_hash_algorithms = _stack(HashAlgorithm.md5, HashAlgorithm.sha1, HashAlgorithm.sha256, HashAlgorithm.sha512, depth=3)
    ...
    _assert_bit_vectors_unique_across_requests(
        ...
        [
            _construct_request_for(
                MaskConfig(
                    token_size=2,
                    hash=HashConfig(function=HashFunction(algorithms=hash_algorithm_lst), strategy=DoubleHash()),
                    filter=filter_type,
                    padding="_",
                )
            )
...
_hardeners = _stack(
    ...
    Rule90Hardener(),
    RandomizedResponseHardener(probability=0.5, seed=727),
        ...
        [
            _construct_request_for(
                MaskConfig(
                    ...
                    filter=filter_type,
                    padding="_",
                    hardeners=[RandomizedResponseHardener(seed=seed, probability=0.5)],
                )
            )
            for seed in (727, 727 * 2, 727 * 3)
        ],
...
def test_different_vectors_with_different_rehash_parameters(test_client, filter_type):
    rehash_params = [(8, 8, 3), (16, 8, 3), (8, 16, 3), (8, 8, 5)]
        ...
        [
            _construct_request_for(
                MaskConfig(
                    ...
                    padding="_",
                    hardeners=[
                        RehashHardener(window_size=rehash_args[0], window_step=rehash_args[1], samples=rehash_args[2])
                    ],
                )
            )
...
def test_different_vectors_for_clk_with_different_salts(test_client):
    attr_confs = [
        StaticAttributeConfig(attribute_name="firstName", salt=AttributeSalt(value="foo")),
        StaticAttributeConfig(attribute_name="lastName", salt=AttributeSalt(value="foo")),
        StaticAttributeConfig(attribute_name="firstName", salt=AttributeSalt(attribute="dateOfBirth")),
        StaticAttributeConfig(attribute_name="lastName", salt=AttributeSalt(attribute="dateOfBirth")),
    ]
    ...
    for attr_salt in attribute_salts:
        _assert_bit_vectors_unique_across_requests(
            ...
            [
                ...
                for attr_confs in _patch_weighted_attribute_configs_with_salt(attr_salt)
            ],
        )


@pytest.mark.parametrize(
    "filter_type", [RBFFilter(hash_values=5, seed=727), CLKRBFFilter(hash_values=5)], ids=["rbf", "clkrbf"]
)
    ...
    for factor in (0.5, 2):
        _assert_bit_vectors_unique_across_requests(
            ...
            [
                ...
                for attr_confs in _patch_weighted_attribute_configs_with_weight(factor)
            ],
        )
    ...
    for factor in (0.5, 2):
        _assert_bit_vectors_unique_across_requests(
            ...
            [
                ...
                for attr_confs in _patch_weighted_attribute_configs_with_average_token_count(factor)
            ],
        )
