...
def _resolve_salt(entity: AttributeValueEntity, attr_config: StaticAttributeConfig | WeightedAttributeConfig | None):
...
def _populate_bitarray(ba: bitarray, value: str, hash_fn: HashFn, hash_strategy: AnyHashStrategy, hash_values: int):
...
def _mask_rbf(mask_req: EntityMaskRequest):
    ...
    attr_name_to_bitarray_size_dict: dict[str, int] = {
        attr_conf.attribute_name: bits.optimal_size(p=0.5, n=attr_conf.average_token_count * hash_values)
        for attr_conf in mask_req.attributes
    }
    ...
    parent_bitarray_size = max(
        int(
            math.ceil(
                ...
                attr_name_to_bitarray_size_dict.get(attr_conf.attribute_name)
...
async def mask_entities(mask_req: EntityMaskRequest) -> EntityMaskResponse:
    ...
    return EntityMaskResponse(
        ...
        entities=[
            BitVectorEntity(id=entity.id, value=bits.to_base64(hardener_fn(entity_ba)))
