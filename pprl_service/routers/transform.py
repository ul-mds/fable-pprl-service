...
def _resolve_transformer(tf: AnyTransformer):
    ...
    if tf_generator_fn is None:
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail=f"unimplemented transformer `{tf}`")

    return tf_generator_fn(tf)


def _try_transform(config: TransformConfig, entity: AttributeValueEntity, value: str, transform_fn: StringTransformFn):
